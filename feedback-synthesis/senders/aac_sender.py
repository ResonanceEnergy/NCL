...
Reads AAC capital reports from aac-reports/ and POSTs them to the NCL Brain.
Validates reports against schema before submission.

Usage:
  python aac_sender.py <report_file.yaml>          # Send specific report
  python aac_sender.py --watch                      # Watch for new reports (continuous mode)
  python aac_sender.py --list                       # List pending reports
"""
...
REPORTS_DIR = SENDER_DIR.parent / "aac-reports"
...
def validate_report(data: dict) -> tuple[bool, Optional[str]]:
    ...
    if not isinstance(data.get("performance_metrics"), dict):
        return False, "performance_metrics must be a dict"
...
async def submit_report(report_data: dict, source_file: Path) -> bool:
    """
    Submit AAC report to NCL Brain.
...
def list_reports() -> int:
    ...
    print(f"\nPending AAC Reports ({len(reports)}):")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="AAC Feedback Sender — Submit capital reports to NCL Brain"
    )
    parser.add_argument(
        "report_file",
        nargs="?",
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Send all pending reports from aac-reports/",
    )
    parser.add_argument(
        "--list",
        action="store_true",
