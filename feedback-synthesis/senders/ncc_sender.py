...
Reads NCC execution reports from ncc-reports/ and POSTs them to the NCL Brain.
Validates reports against schema before submission.

Usage:
  python ncc_sender.py <report_file.yaml>          # Send specific report
  python ncc_sender.py --watch                      # Watch for new reports (continuous mode)
...
REPORTS_DIR = SENDER_DIR.parent / "ncc-reports"
...
async def submit_report(report_data: dict, source_file: Path) -> bool:
    """
    Submit NCC report to NCL Brain.
...
def list_reports() -> int:
    ...
    print(f"\nPending NCC Reports ({len(reports)}):")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="NCC Feedback Sender — Submit execution reports to NCL Brain"
    )
    parser.add_argument(
        "report_file",
        nargs="?",
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Send all pending reports from ncc-reports/",
    )
    parser.add_argument(
        "--list",
        action="store_true",
