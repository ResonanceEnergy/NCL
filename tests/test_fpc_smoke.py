...
_FPC = ROOT / "ncl_agency_runtime" / "fpc"
DATA_PATH = _FPC / "data" / "raw" / "example.csv"
CONFIG_PATH = ROOT / "ncl_agency_runtime" / "fpc" / "config" / "steering.json"
...
def test_base_imports():
    from ncl_agency_runtime.fpc.council.base import ForecastResult, ModelStrategy
...
def test_statsforecast_strategy_init():
    from ncl_agency_runtime.fpc.council.strategy_statsforecast import StatsForecastStrategy
...
def test_ensemble_equal_weights():
    from ncl_agency_runtime.fpc.council.ensemble import WeightedEnsemble
    from ncl_agency_runtime.fpc.council.strategy_statsforecast import StatsForecastStrategy
...
def test_mase_metric():
    from ncl_agency_runtime.fpc.eval import mase
...
def test_smape_metric():
    from ncl_agency_runtime.fpc.eval import smape
...
def test_agent_roles_loaded():
    from ncl_agency_runtime.fpc.agents import LAUNCH_SQUADRON, get_agent
...
def test_burst_config_loads():
    from ncl_agency_runtime.fpc.agents.burst import BURST_RECIPES, load_burst_config
...
def test_burst_cost_check():
    from ncl_agency_runtime.fpc.agents.burst import BurstConfig, can_burst
...
def test_release_policy_loads():
    policy_path = ROOT / "ncl_agency_runtime" / "fpc" / "ops" / "ReleasePolicy.yaml"
