...
def test_version():
    from future_predictor_council.src import __version__
    assert __version__ == "0.3.0"


def test_public_imports():
    from future_predictor_council.src import FuturePredictorCouncil, PredictionHorizon, RiskLevel
...
class TestCouncilOrchestrator:
    def test_init_defaults(self):
        from future_predictor_council.src.council_orchestrator import FuturePredictorCouncil
...
class TestTracker:
    def test_record_and_list(self, tmp_path):
        from future_predictor_council.src.tracker import PredictionTracker
...
class TestReportGenerator:
    def test_generate_council_report(self, tmp_path):
        from future_predictor_council.src.reports import ReportGenerator
...
class TestFlywheelFeed:
    def test_emit_and_read(self, tmp_path, monkeypatch):
        import future_predictor_council.src.flywheel_feed as ff
...
class TestIngestion:
    def test_csv_ingester(self, tmp_path):
        from future_predictor_council.src.ingestion import CSVIngester
    ...
    def test_pipeline_no_sources(self):
        from future_predictor_council.src.ingestion import IngestionPipeline
...
class TestDataSources:
    def test_fred_ingester_no_key(self):
        from future_predictor_council.src.data_sources import FREDIngester
        ing = FREDIngester(api_key="")
        signals = ing.fetch_series("GDP")
        assert signals == []

    def test_fred_default_indicators(self):
        from future_predictor_council.src.data_sources import FRED_INDICATORS
    ...
    def test_alpha_vantage_no_key(self):
        from future_predictor_council.src.data_sources import AlphaVantageIngester
...
class TestExplainability:
    def test_explainer_init(self):
        from future_predictor_council.src.explainability import ForecastExplainer
...
class TestProphetStrategy:
    def test_import_and_name(self):
        from future_predictor_council.src.council.strategy_prophet import ProphetStrategy
        s = ProphetStrategy()
...
class TestConfig:
    ...
    def test_steering_config_still_exists(self):
        path = ROOT / "config" / "steering.json"
