...
class WatchtowerAgent(BaseAgent):
    ...
    def _execute(self, task: Task, event: dict[str, Any]) -> dict[str, Any]:
        ...
        # Matrix Monitor integration — pull unified health data if available
        ...
        try:
            ...
            if latest:
                matrix_data = {
                    ...
                    "checks_total": latest.get("checks_total", 0),
                    "slo_violations": latest.get("slos_in_violation", 0),
                }
                # Upgrade alert level based on Matrix Monitor score
                mm_score = latest.get("overall_score", 1.0)
                if mm_score < 0.3:
                    level = "red"
                elif mm_score < 0.5 and level != "red":
                    level = "yellow"
        except Exception:
            pass  # Matrix Monitor not yet initialized — graceful degradation
