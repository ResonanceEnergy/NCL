...
class GoldenTask(BaseModel):
    ...
    difficulty: TaskDifficulty = Field(default=TaskDifficulty.STANDARD, description="Task difficulty")
    ...
    expected_output: dict[str, Any] = Field(
        ...
        description="Expected structured output (for value matching)"
    )
    expected_keys: list[str] = Field(
        default_factory=list,
        description="Keys that MUST exist in output"
    )
    expected_patterns: list[str] = Field(
        default_factory=list,
        description="Regex patterns output string values must match"
    )
    expected_type: Optional[str] = Field(
        default=None,
        description="Expected output type name (e.g., 'dict', 'list', 'str')"
    )

    # Failure conditions
    failure_conditions: list[str] = Field(
        default_factory=list,
        description="Error messages that constitute failure"
    )
    max_duration_ms: float = Field(
        default=30000,
        description="Maximum allowed execution duration in milliseconds"
    )
...
class SuiteResult(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="Suite execution timestamp"
    )
