"""council_pack.replay — relocated from council_runner (W5-06, 2026-05-23).

Deterministic Replay Engine for legacy Council Runner records. Saves
runs to disk, loads historical records, and replays with comparison
reports.

Replay uses the legacy ``run_parallel_council`` (Planner/Skeptic/Risk)
which now lives in ``council_pack.legacy``. New surfaces should use the
v2 pack instead — but historical run records (from
``/council-runner/runs``) still need to be replayable, which is what
this module supports.

Originally lived at ``runtime/council_runner/replay.py``.
"""
...
from .legacy import run_parallel_council

log = logging.getLogger("ncl.council_pack.replay")
