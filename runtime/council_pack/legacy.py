"""council_pack.legacy — Planner/Skeptic/Risk back-compat shim (W5-06).

This module is the **deprecated** fallback path for ``/council-runner/run``
and the **active** path for ``ReplayEngine.replay()`` over historical
v1 records. It preserves the v1 three-agent parallel council
(Planner / Skeptic / Risk) so replays of historical runs continue to
produce comparable outputs and the ``/council-runner/run`` fallback
keeps working when the pack path errors.

**Do not call this from new code.** New surfaces should use
``council_pack.runners.run_council_with_pack`` — the universal pack
debate (Delphi-MAD 6-member roster + MMR + temporal split +
contradictions + calibration + peer review + 3-tier write-back).

History
-------
The full v1 module lived at ``runtime/council_runner/agents.py`` and
was archived to
``archive/strike-point-pre-merge/council_runner/agents.py`` on
2026-05-23 (W5-06). ``run_parallel_council`` is the only public symbol
kept alive here.
"""
...
log = logging.getLogger("ncl.council_pack.legacy")
...
async def _get_http_client() -> httpx.AsyncClient:
    """Return (and lazily create) the shared httpx client for the legacy runner."""
...
def get_agent_configs() -> dict[AgentRole, AgentConfig]:
    """Return the three agent configurations for the legacy council."""
...
async def run_parallel_council(
    ...
    """
    DEPRECATED legacy v1 fallback. Use
    ``council_pack.runners.run_council_with_pack`` for new code.

    Execute all three legacy agents in parallel: Planner, Skeptic, and
    Risk are run concurrently via asyncio.gather(), their outputs merge
    into consensus, and full provenance is recorded.

    Retained because:
    - ``/council-runner/run`` falls back to this when the pack path
      errors, so the endpoint never regresses.
    - ``ReplayEngine.replay()`` deterministic-replays historical v1
      records via this function.
    """
    warnings.warn(
        "council_pack.legacy.run_parallel_council is deprecated. "
        "Use council_pack.runners.run_council_with_pack for new code.",
        DeprecationWarning,
        stacklevel=2,
    )
