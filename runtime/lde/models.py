...
class TradingInsight(BaseModel):
    ...
    superseded_by: Optional[str] = Field(default=None, description="ID of newer insight that replaces this")
...
class DoctrineRule(BaseModel):
    ...
    supporting_insights: list[str] = Field(default_factory=list, description="Insight IDs that support this rule")
    contradicting_insights: list[str] = Field(default_factory=list, description="Insight IDs that challenge this rule")
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    updated_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    expires_at: Optional[datetime] = Field(default=None, description="When this rule should be re-evaluated")
...
class TrendMonitor(BaseModel):
    ...
    description: str = Field(default="")
    category: InsightCategory = Field(...)
    direction: str = Field(default="emerging", description="emerging, accelerating, peaking, declining, reversing")
    ...
    watch_triggers: list[str] = Field(default_factory=list, description="Conditions that should trigger action")
...
class LivingDoctrine(BaseModel):
    ...
    # Metadata
    top_tickers: list[str] = Field(default_factory=list, description="Most frequently referenced tickers")
    market_bias: str = Field(default="neutral", description="Overall doctrine bias: bullish, bearish, neutral, mixed")
    confidence_score: float = Field(default=5.0, ge=0.0, le=10.0, description="Overall doctrine confidence")
