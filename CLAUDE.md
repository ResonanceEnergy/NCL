...
**Status**: Authoritative spec, rewritten 2026-05-23, updated 2026-05-25 (Wave 14A-G arc), 2026-05-29 (Wave 14X + 14Y arcs), 2026-05-30 (Wave 14AA-14BT arcs), and 2026-05-31 (Wave 14CV-14CY + 14DA-14DI arcs — see top block below) — supersedes pre-retirement docs in `archive/docs-pre-retirement/`.

**Wave 14DA-14DI arc (2026-05-31 late session)** — Renderer parity + Watchlist + Reddit allowlist + Dashboard 8-tile grid:

- **Wave 14DA** — Portfolio WATCHLIST tab (between PORTFOLIO and AGENT) via WKWebView embed of TradingView watchlist 330949579. PAPER tab cleaned to filter `strategy=="snapshot"` rows. RotationRRGView gains header timestamps + per-sector day-% chip below each dot.
- **Wave 14DB** — Watchlist always in Intel context. NEW backend `GET/POST /watchlist/tickers` persists to `data/watchlist/tickers.json`. iOS WatchlistView ticker editor POSTs on save. AWAREBOT scorer extended: `compute_situational_relevance` gains `tickers_in_watchlist` kwarg (+0.45 boost — strongest of all situational factors). `_get_situational_ctx` loads watchlist via fresh import + 30-min TTL. Score loop stamps `signal.metadata["watchlist_hit"] = "<ticker>"` on match.
- **Wave 14DC** — Dashboard brief sheet (`BriefFullDocumentSheet`) routed through shared `ProfessionalMarkdownView` (was primitive `parseBlocks` that dumped wall-of-text). Long-value kvRow detection (>40 chars OR keys like thesis/structure/notes) wraps to next line in cyan mono.
- **Wave 14DD** — `brief_archiver.py::_lane_to_md` rewritten with recursive `_lane_field_to_md` + `_render_dict_as_bullet` helpers. Previous dict branch did `f"- **{k}**: {v}"` which printed Python repr for nested list-of-dicts (reddit_top10 became one giant blob). Now: list → bullets, dict-in-list → sub-bullets per key, primitives → inline. Each Top10/MOVES/XREF row renders as own bullet with sub-bullets.
- **Wave 14DE** — Executive brief mode in `BriefIntelConsolidatedView` routed through `ProfessionalMarkdownView` via new `composeExecutiveMarkdown()` — was rendering custom Card stack only. Now identical visual hierarchy across Morning/Executive/Debrief.
- **Wave 14DF** — Night Watch view rewrite. NATRIX: "i dont want like to json i want to see the data formatted professionally". Replaced `bulletCard` + `rawDiagnosticsDrawer` + raw-markdown ScrollView footer with single `ProfessionalMarkdownView` driven by `composeNightWatchMarkdown()` (status accent color matches statusPill). Empty-state fallback specifically pinpoints Wave 14CX diagnostic path (`/system/silent-failures` → `night_watch_triage_failed`). Five surfaces now share one renderer: Morning · Executive · Debrief · Night Watch · Dashboard sheet.
- **Wave 14DG** (`fc794be`) — Reddit Top10 allowlist filter. NATRIX: "why is reddit top 10 pulling random subreddits and not top 10 out of the 55 subreddits i told you to follow". Two breaks: (1) `watch_queries.json` had been pruned to 10 tier1 subs in Wave 14CI (volume reduction). Restored full 55-sub list (T1=10, T2=16, T3=29). (2) `_collect_reddit_top10` in brief_prep.py had no allowlist gate — Reddit `search.rss` returns site-wide hits, so keyword queries pulled in indiegames/Layoffs/TechProfitLab/u_seoahmedabad123/tldrAI. Added load-once allowlist at top of function + `if sub.lower() not in allow: continue` gate before every row append. Live verified post-fire: Top10 only shows Daytrading/StockMarket/economy/pennystocks.
- **Wave 14DH** (FirstStrike `dfec9e0`) — Dashboard 8-tile QUICK ACTIONS grid. NATRIX: "for dashboard i want the squares: Brief Intel Portfolio Calender journal memory strike point pump prompt". Exactly 8 tiles in stated order, each routes to canonical surface: Brief→Intel/.brief · Intel→Intel/.focus · Portfolio→Portfolio · Calendar→Calendar · Journal→Journal · Memory→Memory · Strike Point→Dashboard/.strikePoint · Pump Prompt→Dashboard/.chat. iPad 4-cols, iPhone 2-cols. BriefLandingCard stays above grid.
- **Wave 14DI** (current) — Session close: brain bounced, iOS rebuilt on all 4 targets, CLAUDE.md updated both repos.

---

**Wave 14CV-14CY arc (2026-05-31 mid-session)** — **BRIEF priority + Intel audit-fix arc**:

NATRIX flagged the Dashboard brief as "5 boxes not one doc", missing PM debrief surfacing, lots of skipped trades, and Intel-tab content drift. Five-wave arc:

- **Wave 14CV** (`52fd29c`) — **BriefLandingCard single-document UI + PM debrief .md archive**. Replaced the 5-tile LazyVGrid with a unified Markdown renderer fetching `/intelligence/briefs/archive/{date}/{am,pm}`. PM debrief wired into archive via `_adapt_pm_to_envelope()` + `brief_archiver.archive_brief(mode="pm")`. NEW `BriefFullDocumentSheet` parses .md into H1/H2/H3/text blocks. NEW `BriefMarkdownComposer` fallback for legacy JSON. Investigation finding: yesterday's PM debrief DID fire — what NATRIX saw was iOS not surfacing it well through the old 5-tile grid.

- **Wave 14CW** (audit doc) — **9-surface Intel tab audit**. Live-fired audit of NOW/STREAM/XREF/MOVES/BRIEF/NIGHTWATCH/AGENDA/FOCUS/PREDICTIONS. Verdict: 0 GREEN / 4 YELLOW / 4 RED. Worst 3: PREDICTIONS (all 20 single-model deepseek with hardcoded 0.0486 confidence, zero recorded outcomes), NIGHT WATCH (STATUS=UNKNOWN, every Sonnet triage 400), STREAM (58/80 items Reddit promo spam). Plus smoking guns: XREF `NOW` false-positive matching "now" in YTC titles; MOVES `youtube_search` shared-divisor producing identical 4.52 ratio on all 23 alerts. Deliverable: `outputs/INTEL_AUDIT_2026-05-31.md` (~3,200 words, top-10 punch list).

- **Wave 14CX** (`ece8605`) — **Audit fix batch (5 items)**. (1) Reddit watch queries swapped from 6 AI-business themes to 6 trading themes (XRP / silver squeeze / Fed rate cut / options flow unusual / GME short squeeze / capital rotation sectors). (2) Night Watch 400 root cause: Python operator-precedence bug at `runtime/autonomous/night_watch/analyst.py:535-544` — ternary `if deterministic_issues else "None"` applied to the WHOLE multi-line `cost_analysis` prompt, collapsing it to the 4-char string "None". Scoped ternary + `prompt_len >= 16` guard + `resp.text` logged on 4xx. (3) MOVES per-source divisor: Wave 14CR computed `days_present` ONCE globally; now `_src_divisors(source)` per-source. Live verified: youtube_search 23→0 alerts (correctly — 1-day data); reddit 16 alerts/15 unique ratios. (4) `/intelligence/digest` freshness flag: legacy `latest_brief.json` hasn't rotated since 2026-05-29; endpoint now computes `brief_age_hours`, sets `stale: true` when >12h, reads today's Pro Brief into `fresh_pro_brief.{headline,lanes,generated_at}`. (5) XREF `NOW` false-positive: NEW `_AMBIGUOUS_REQUIRE_DOLLAR` set in `ticker_universe.py` covering NOW/ALL/ANY/ON/IT/AI/BIG/MAN/WIN/U/X + FLOW/SAND/OP/POL — these require explicit `$TICKER` notation. Wired into cross_reference + trend_tracker + a read-time filter on `/intel/convergence`. Live verified: XREF 7→6 cards (NOW dropped).

- **Wave 14CY** (current) — **BRIEF priority + Intel reorder + professional formatting**. NATRIX: "morning brief executive brief and afternoon debrief need professional formatting, and all listed under brief in intel, also BRIEF should be first button in top row and given priority status". (i) `IntelSection.allCases` reordered with `.brief` as position #1 (was hidden in trial row). (ii) NEW `priorityBriefButton` hero CTA renders above `FSSectionPicker` with orange accent + PRIORITY chip + doc icon + "Morning · Executive · Debrief" subtitle. Trial row no longer includes BRIEF (left with `[nightWatch, agenda, focus, predictions]`). (iii) `briefSection` replaced with `BriefIntelConsolidatedView` — NEW `Sources/Views/Intel/BriefIntelConsolidatedView.swift` (~480 LOC) with 3-mode card picker (Morning · Executive · Debrief), each with distinct accent (orange/cyan/purple) + icon (sunrise.fill/doc.text.magnifyingglass/moon.stars.fill). Morning + Debrief render through NEW `ProfessionalMarkdownView` (block parser → H1/H2/H3/paragraph/bullet/rule with accent-colored H2 headers + rule underneath + key=value bullet detection rendering prices in monospace cyan). Executive renders structured cards: stale-warning banner, fresh-Pro-Brief card, KEY SIGNALS, RISK ALERTS, SOURCE BREAKDOWN.

**Scheduler unchanged post-14CY** — 36 unique `ncl-*` task names. All 4 iOS targets rebuilt + deployed via `xcodegen generate` then `xcodebuild`.

---

**Wave 14CP-14CU arc (2026-05-30 / 2026-05-31 overnight)** — AWAREBOT + TRADERAGENT audit + auto-trader unblock:

- **Wave 14CP** — Full data-flow quality audit via subagent. Findings: TRADERAGENT had never opened a single paper trade (106/106 reasoning chains REJECT); Pro Brief never registers trade_ideas with tracker (audit showstopper #1); chair LLM emits literal "sig_id" placeholders as citation values. Doc: `outputs/AWAREBOT_TRADERAGENT_FLOW_AUDIT_2026-05-30.md`.
- **Wave 14CQ** (`7ed7345`) — Brief unifier — `.md` archive + memory snapshot + auto-trader bridge. NEW `runtime/intelligence/brief_archiver.py` (~460 LOC). Closes audit B4.1 + B4.2.
- **Wave 14CR** (`f65a213`) — Audit punch-list batch: TradeIdea.sources top-level field, trend_tracker baseline divisor fix, news_trends_double rule hardening, options_flow ticker enrichment, scheduler typo fix, memory/tracker fire-and-forget. Cuts 60% false `fading_100pct` alerts.
- **Wave 14CS** (`2e16789`) — Audit-final 3: BERTopic verify (no fix needed, theme converges only emit 4 hardcoded clusters), NAV race fix (governor + loop now see same band), silent-failure counters at `/system/silent-failures` for AuthorityLearner / EntityClusterer / situational / persist-dedup / promo-marker / BERTopic load.
- **Wave 14CT** (`1cbc390`) — Auto-trader threshold/schema audit → **FIRST PAPER TRADE OPENED in 4 days** (GRRR qty=100 @ $21.02, R=$150). Five compounding fixes: brief stocks now strategy=goat (was "unknown"), R_per_share computed from |entry-stop|, options skipped (brief schema ambiguous), null-priced ideas skipped, `stop_type="price"` default.
- **Wave 14CU** (`68eb76a`) — Threshold tunes: min_R_R_ratio 1.5→1.25, NCL_BETA_ADJ_MAX_NET_PCT 50→80, purge 15 stale options ideas, full options schema in chair prompt (option_strike + option_dte + option_right + premium + underlying_entry/_target/_stop) + re-enable options registration with complete-schema gate.
...
- **14X-1B** `de65087` — Yesterday's Recap binding + Wave 14S 12-block sprawl cut. NATRIX's flagship fix for "morning brief got really weak and lost its format" — closes the missing feedback loop. NEW `collect_yesterday_recap()` in `brief_prep.py` reads `data/portfolio/auto_trader/eod_summaries.jsonl` + yesterday's brief output. Returns `{date, ideas_given, closes, winners/losers/scratches, total_r, tickers_closed, drift_signals, lesson}`. Wired into `build_prep_pack` as `pack['yesterday_recap']`. Chair prompt now includes it in PREP CONTEXT + emits `yesterday_recap` JSON block. NEW YESTERDAY'S RECAP section in `brief_presenter` renders FIRST (above MARKET OPEN PLAN). Plus: Wave 14S's "DAILY CONTEXT — 12 blocks" dump cut to 4 canonical blocks (PORTFOLIO / AGENT / CONTEXT / TODO_7DAY). OPTIONS/CRYPTO/POLYMARKET/YTC/GOAT/BRAVO/PREDICTIONS dropped from Brief render — they have their own iOS tabs.

- **14X-2** `031a692` + **14X-2b** `4d35130` — Real multi-provider brief council. NATRIX called out that the 4-Sonnet placeholder was 5× the cost with zero diversity (4 copies of the same brain debating itself). New council per the original Delphi-MAD spec:
  ```
  Macro    Claude Opus 4         heavy macro reasoning
  Pulse    Grok-4 (xAI)          real-time sentiment / X / news
  Flow     Gemini 2.5 Flash      flow analysis (GOOGLE_API_KEY)
  Tech     GPT-4o (OpenAI)       chart setups / momentum
  Chair    Claude Opus 4         heaviest synthesis
  ```
  NEW `_xai_call` / `_openai_call` / `_gemini_call` helpers in `brief_council.py`. NEW `_dispatch_call` routes by model-name prefix (`claude-` / `grok-` / `gpt-` / `gemini-`). Cost change: ~$0.42/brief vs old $0.26 (+62%) — but now actual diverse reasoning, not noise. End-to-end verification blocked on Anthropic account balance (separate from daily-budget) being exhausted today.

- **14X-3** `d82e702` — Push notifications re-wired + Strike Point dead-code deletion. Honest read of the codebase per NATRIX's "stop winging it, read internal files" found 2 silent bugs since the 2026-05-23 strike_point_orchestrator archive: `signal_processor._push_alerts` and `scheduler.py` journal-reflection block both `try: from ..strike_point_orchestrator import notify_*` followed by `except ImportError: log.warning(...)` — the file doesn't exist anymore, so 6+ days of intel push alerts + daily reflection pushes have been silently dropped. Re-wired both through `runtime/notifications/alert_dispatch.enqueue_alert` (the central AlertDispatcher singleton, 1/10s rate limit + 1h dedup). Also: `/notifications/test` endpoint was returning HTTP 410 ("push helper retired") — now wired through `enqueue_alert`, returns 200 + fires a real ntfy push. Dead-code deletions: lane_router `("strike-point", PORTFOLIO)` mapping (nothing emits with that source); `STRIKE_POINT_KEYWORDS` list + `_strike_point_score` function in scraper.py (vestigial from retired strike-point pillar, replaced by Wave 14X-1A date-sort).

- **14X-Y Phase 1B-3** `3b00990` — Cross-Reference Engine (the breakthrough mechanism NATRIX described). NEW `runtime/cross_reference/__init__.py` (~330 LOC) scans the AWAREBOT signal stream for converging signals and promotes candidates so TRADERAGENT can evaluate without drowning in the loose AWAREBOT pool. Three rules: (1) ticker convergence (same ticker in ≥2 distinct AWAREBOT sources within 4h), (2) theme convergence (shared keyword cluster across ≥3 distinct sources within 24h — 5 themes: rate_policy / ai_capex / energy_supply / crypto_macro / geopolitical), (3) news+trends double-verifier (NATRIX's "if it's in the press AND spiking in search, that's confirmation"). Dedup by `(ticker, day)` so same hot ticker doesn't re-promote hourly. Writes `data/cross_reference/promotions.jsonl`. Scheduler: NEW `_cross_reference_loop` at 300s cadence (`ncl-cross-reference`). Pure pull-from-disk, no LLM cost.

- **14X-Y Phase 1B-4** `cdd6335` — Situational context into Brief + AWAREBOT scorer. NEW `collect_situational_context(brain)` in `brief_prep.py` returns `{lunar, calendar_today, tickers_with_event_today, journal_posture, tickers_in_journal_today}` — wired into pack + chair prompt PREP CONTEXT (budget 3500→4000 chars). AWAREBOT scorer extended: NEW 7th factor `W_SITUATIONAL_RELEVANCE = 0.05` (W_NOVELTY halved 0.10→0.05 to keep sum = 1.00). NEW `compute_situational_relevance(signal_text, tickers_in_journal_today, tickers_with_calendar_event_today, morning_quiz_focus)` helper scores signal matches against NATRIX's current life context. `compute_composite_score` gains `situational=0.0` kwarg (backward-compatible — no-op until callers thread context).

- **14X-Y Phase 2** `a2bc936` — Afternoon Debrief surface (NATRIX's second daily anchor). NEW `runtime/intelligence/afternoon_debrief.py` (~250 LOC) — single Opus call via `_dispatch_call`, no 4-member council. Reads today's EOD summary + today's brief + today's cross-ref promotions + rotation today-vs-yesterday. Synthesizes 6-tile structure (`headline / today_scoreboard / night_watch_focus / agent_reasoning_highlights / post_market_scan / rotation_shift / one_q_quiz_prompt`). Persists to `data/afternoon-debrief/YYYY-MM-DD.json`. ~$0.08/run. Scheduler: NEW `_afternoon_debrief_loop` fires at 16:30 local clock (`ncl-afternoon-debrief`). REST: `GET /intelligence/afternoon-debrief/today` + `/latest` + `POST /fire`.

- **14X-Y Phase 5** `4c43f72` — AWAREBOT + Cross-Reference mandate docs. `docs/AWAREBOT_MANDATE.md` codifies the intel-camp identity (7-factor scoring with SITUATIONAL factor, source priority order per NATRIX direction: YTC #1 / Reddit #2 / X #3 paused / MARKETS+POLYMARKET merged at #5 / Trends/News verifier-only, authority ceiling at LLM_SINGLE except council-grade YTC). `docs/CROSS_REFERENCE_MANDATE.md` codifies the engine bridging AWAREBOT → TRADERAGENT (3 rules, AWAREBOT-source whitelist, consumers list). Original `INTEL_MANDATE.md` remains as the UI-lane mandate (what iOS Intel tab is); these are the agent-camp mandates (what the agents do).

**REVAMP architecture context** (full doc at `outputs/REVAMP_2026-05-29.md`): the 14X arc implements NATRIX's two-camp split — **AWAREBOT** (intel/entertainment, loose, never trades) and **TRADERAGENT** (portfolio/serious, tight 15-gate chain, paper-only) — sharing one data substrate, connected by the new Cross-Reference engine. The Brief became Dashboard's situational cockpit (TIME / SELF / BRIEF / AGENT bands per NATRIX's "dashboard isn't just centered around brief … incorporate sun moon calendar journal"). Morning Brief Pro = 05:30 ET anchor; Afternoon Debrief = 16:30 ET anchor. Phases 3+4 (iOS Dashboard rebuild + Intel restructure to YTC #1 order, MARKETS+POLYMARKET merged) deferred to next session.

**Scheduler loop count post-14X-Y**: +2 new (`ncl-cross-reference` 5min, `ncl-afternoon-debrief` 16:30 local) on top of the 43-loop baseline = ~45 unique `ncl-*` task names. End-to-end Brief verification still blocked on Anthropic account balance (daily-budget headroom exists but account credits are exhausted — separate billing issue, not code).

**Recent wave summary (2026-05-26)**:

- Wave 14K — Auto-Trader Agent (paper-trading only, never live). Full 8-phase arc shipped 2026-05-26. **Hard line preserved**: NCL never places live orders. The auto-trader runs a closed-loop on `PaperTradingEngine`; "graduation" is decision support for the operator, never auto-promotion. Architecture doc at `docs/AUTO_TRADER_AGENT_2026-05-26.md`. New package `runtime/portfolio/auto_trader/` (~2,500 LOC across 11 modules):
  - **Phase 1 (K0a/b/c)** — `state.py` (operator-controlled active/paused/drawdown_halt state, default OFF, REST POST /resume to enable), `policy.py` (20+ tunable entry-criteria thresholds: min_R_R_ratio=1.5, max_opens_per_tick=3, max_opens_per_day=12, allow_counter_trend=False), `observability.py` (per-trade reasoning-chain capture: prompt + model + sources + governor decision, JSONL append + JSON cache, idempotent on trade_idea_id). Commit `928c48e`.
  - **Phase 2 (K1a/b/c/d)** — `loop.py` main decision loop (60s market / 300s off-hours). For each new emitted idea: governor check → policy auto-bar → friction injection → paper.create_trade → reasoning chain → memory unit at importance 75. Idempotent + restart-safe. K1b stitches `trade_idea_id` into `paper_trade.scanner_data` so outcome attribution can reconnect on close. Commit `01984e7`.
  - **Phase 3 (K2a/b/c/d)** — `price_feed.py` live-quote feeder (30s market / 300s off, runs even when auto-trader paused — pause stops new opens, not mark-to-market; self-heals on empty symbols / all-quotes-fail / engine exception). `outcome_attributor.py` paper-close → trade_idea_tracker.update_outcome → memory unit at importance 80. TRIGGER_TO_OUTCOME map (stop_hit→stopped_out etc.). Reconciles R_multiple drift between engines. Commit `5d427b4`.
  - **Phase 4 (K3a/b/c/d)** — `strategy_bandit.py` Beta-Bernoulli + Thompson sampling over strategies, credible_interval via scipy or normal-approx fallback, ranked_by_credible_lower_bound for risk-averse strategy selection. `shap_attribution.py` Shapley-flavored conditional-win-rate analysis (no `shap` dep): per-feature lift in hit rate over closed trades, top-3 positive/negative predictors, MemUnit at importance 90 every 10 closes per strategy. Commit `457970f`.
  - **Phase 5 (K4a/b/c/d)** — `self_research.py` (~330 LOC) closes the meta-loop. K4a `apply_shap_to_authority_learner` — SHAP per-source lifts ≥0.10 push synthetic correct/wrong outcomes into `SourceAuthorityLearner` so winning sources gain authority weight on next brief. K4c `generate_research_topics` — clusters losing trades by (sector_etf, source, stop_type, rotation_quadrant); ≥3 losses → human-readable topic ("Why did 4 XLE-sector trades lose"). K4d `brief_context_packet` — 3-block text packet (strategy expectancy + strategy learn + open research topics) auto-prepended to brief executor prompt every morning so the brief biases trade-idea allocation toward higher-LCB strategies. Bug caught + fixed in test authoring: `learner.record_outcome()` → `learner.record(source, outcome, *, delta, notes)`. Commit `6cb7888`.
  - **Phase 6 (K5a/b/c/d)** — `drift_detector.py` (~280 LOC) Page-Hinkley concept-drift detector on the win/loss stream per strategy. Welford incremental sample mean as reference (caught + fixed an EWMA-bootstrap bug producing false positives on stationary 50/50 data). m_down/m_up cumulative test statistics, MIN_N=20 warmup, PH_LAMBDA=0.50. STABLE / DRIFT_DOWN / DRIFT_UP. K5b auto-pause on DRIFT_DOWN transition flips `paused_by="drift_detector"` and emits `portfolio:strategy_drift` MemUnit at importance 90. `graduation_gate.py` (~290 LOC) multi-criteria readiness report with 8 weighted criteria: min_sample_size (N ≥ 50, weight 3), min_hit_rate (≥45%), min_profit_factor (≥1.5), min_sqn (≥1.7), min_expectancy_R (≥0.10), min_lcb_hit_rate (Bayesian ≥40%), no_recent_drift (14d window), cycle_phase_confident (≥0.35). Commit `cef3b10`.
  - **Phase 7 (K6a/b/c)** — `friction_profile.py` (~300 LOC) per-strategy `FrictionProfile {slippage_bps, partial_fill_prob, partial_fill_min_pct, asset_type}`. Per-asset defaults: stock 3 bps / options 50 bps + 5% partial / crypto 10 bps / futures 15 bps. K6a `apply_friction_to_payload()` shifts long entries higher / short entries lower; probability-gated partial fills reduce qty to sampled fraction. Wired into `loop._idea_to_paper_payload` so every auto-trader open reflects realistic execution cost. K6b `calibrate_from_closes` EMA-blends observed slippage into the profile (learning rate 0.1); `maybe_calibrate` auto-fires every 10 closes from outcome_attributor. K6c `/auto-trader/dashboard` rollup endpoint (8-key aggregate: state + top strategies + drift + graduation + research topics + recent closes + friction summary) for iOS PaperTradingView consumption. Commit `a93ff73`.
  - **Phase 8 (K7a/b/c/d)** — K7a circuit breakers from `hygiene.CircuitBreaker` (Wave 14J J8c) wrapped around 4 external deps in the auto-trader loop: `auto_trader:drawdown_bucket`, `auto_trader:risk_governor`, `auto_trader:trade_idea_tracker`, `auto_trader:paper_engine`. 3 consecutive failures opens the breaker for 10 min (auto-resets). Governor failure defaults to REJECT (safest: no sizing without a gate). Tracker failure skips the tick. K7b crash-recovery test verifies state.json + drift_state.json + friction_profiles.json + research_topics.json all replay correctly after a simulated process restart. K7c full lifecycle integration test exercises every wave-14K module end-to-end (emit → friction → close → bandit → drift → cluster → graduation). New REST endpoint `/auto-trader/circuit-breakers` shows live breaker status.

  REST surface (under `/portfolio/auto-trader/*`): status, pause, resume, expectancy, dashboard (8-key rollup), drift / drift/{strat} / drift/{strat}/reset, graduation / graduation/{strat}, research/topics + generate + {id}/resolve, brief-context-packet, friction + friction/{strat} + friction/{strat}/calibrate, circuit-breakers — total **21 endpoints**. Scheduler: 2 new named tasks `ncl-auto-trader-loop` (60s market / 300s off) + `ncl-auto-trader-prices` (30s market / 300s off) — grep confirms **43 unique `ncl-*` names** in `runtime/autonomous/scheduler.py` post-14K (was 35 in 14G, +8 across 14H/14I/14J/14K). Tests: **40 auto-trader tests** in `tests/test_portfolio_manager.py`, all green.

- Wave 14I item 10 — iOS RRG widget shipped (commits NCL `696eeca`, FirstStrike `e75c980`). Visual surface for the rotation backend: new `IntelSection.rotation` case wires `RotationRRGView` (~330 LOC SwiftUI: header card, 4-quadrant ZStack chart with sector dots plotted at normalized (ratio_pct_chg_20d, rs_momentum_5d), breadth chip, leadership summary, quadrant breakdown, style ratios card, cycle phase card) into the Intel tab. Backend: 2 new endpoints — `GET /intelligence/rotation` (loads today's rotation/style/cycle, 404 if no snapshot yet) and `POST /intelligence/rotation/fire` (manual dev/ops trigger; `asyncio.gather` of build_rotation_snapshot + build_style_snapshot + build_cycle_phase_snapshot). New iOS files: `Sources/Models/RotationSnapshot.swift` (9 Decodable structs mirroring the envelope), `Sources/Network/NCLBrainClient+Rotation.swift` (fetchRotation + fireRotationBuild via BrainError), `Sources/Views/Intel/RotationRRGView.swift`. Live-deployed to iPhone 16e sim + iPad Pro 13" M5 sim + physical iPhone 15 Pro Max + physical iPad Pro 11". Live read confirmed: keys=[date, rotation, style_ratios, cycle_phase], XLK leading, breadth 81.8%, phase=late_cycle. Closes the Wave 14I deferred RRG widget item.

- Wave 14I — Capital Rotation Tracker (NightWatch → Council). NATRIX research deep-dive on capital rotation surfaced the gap: NCL had the plumbing (sector tags, SectorSnapshot aggregation, ETF watchlist, rule 7a) but no time-series rotation state. Built three new intelligence modules under `runtime/intelligence/`:
  1. `rotation_tracker.py` (~280 LOC) — daily snapshot of 11 SPDR sector ETFs (XLK/XLF/XLE/XLV/XLI/XLP/XLY/XLB/XLU/XLC/XLRE) vs SPY benchmark. Computes per-sector daily % change, JdK-style RS-Ratio (20-day relative-strength ROC vs SPY), RS-Momentum (5-day ROC of the ratio), 4-quadrant RRG classification (Leading / Improving / Weakening / Lagging), 50d-SMA breadth %. Cycle-hint derived from leadership composition (cyclical/defensive/growth/energy). Persists to `data/rotation/YYYY-MM-DD.json`.
  2. `style_ratios.py` (~150 LOC) — IWM/SPY (small-cap vs large-cap), IWD/IWF (value vs growth), XLU/SPY (defensive bid), RSP/SPY (equal-weight vs cap-weight, mega-cap concentration), ARKK/SPY (speculative appetite). Each ratio gets 1d/5d/20d % change + direction tag (rotating_in/out/trending_up/down/neutral). Persists to `data/rotation/style-YYYY-MM-DD.json`.
  3. `cycle_phase.py` (~280 LOC) — business-cycle phase classifier. Reads yield curve (10y vs 5y via ^TNX/^FVX), MANEMP/PMI proxy + jobless claims + HY credit spread (BAMLH0A0HYM2) from FRED. Vote-based 4-state classifier: early_expansion / mid_cycle / late_cycle / recession (+ "mixed" when confidence <0.35). Returns expected_leaders per phase (e.g. late_cycle → XLE/XLB/XLP/XLU). Persists to `data/rotation/cycle-YYYY-MM-DD.json`.

  Integration: `brief_prep.py` now gathers rotation/style/cycle concurrently with futures/VIX/headlines and folds into the prep pack (`rotation_snapshot`, `style_ratios`, `cycle_phase` fields). `brief_council.py::_macro_prompt` passes these three blocks to the Macro Analyst so it can write regime-aware direction calls. Chair adds **rule 7d** — trade ideas should mostly lean WITH the Leading-quadrant sectors; counter-trend ideas must be labeled and justified. `brief_presenter.py` adds a **ROTATION REGIME** sub-block to the Market Open Plan with cycle phase / leading sectors / weakening sectors / breadth % / active style rotations / one-liner read; synthesizes from prep pack if chair omits. GOAT/BRAVO scanners in `routes.py` now tag each result with `rotation_aligned: bool` (true if the result's sector ETF is in today's Leading quadrant) + `sector_etf` field; scan `_meta` includes `rotation_leading_sectors` for operator visibility.

  Live-fired end-to-end: 76s wall, 4/4 members succeeded, market_open_plan now includes `rotation_regime` keys (current_phase, leading_sectors, weakening_sectors, breadth_pct, active_style_rotations, one_liner). Live read: phase=late_cycle, XLK leading, breadth 81.8% (broad), growth/risk-on. Council confidence 0.78. Contradictions resolved included "Late-cycle positioning vs tech leadership: Current momentum overriding defensive cycle expectations" — confirms the Macro Analyst is reading the cycle_phase block and using it. Full research + roadmap doc at `docs/CAPITAL_ROTATION_2026-05-26.md`.
