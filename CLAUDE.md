...
- **Full autonomous hardening (4 systems, May 21)**: (1) Night Watch — nightly 2am ET health audit checks all services, scheduler tasks, Awarebot sub-tasks, staleness windows, LLM provider connectivity, disk space, cost budgets. Pushes green/yellow/red summary via ntfy. (2) Comprehensive cost tracking — wired `record_cost()` to all 20+ previously untracked LLM call sites (Awarebot predictions, memory scoring, entity extraction, intel summaries, Perplexity/GPT/Copilot council members, UNI research, X/Twitter analysis, war room, swarm, user chat). Added `perplexity` to budget config. (3) ntfy alerting — availability tracker alerts (regression, latency spikes, workflow down) now push via ntfy with severity-mapped priorities. Cost tracker budget warnings (80%) and hard caps (100%) now push ntfy alerts with daily rate limiting. (4) Supervisor self-healing — new `_supervisor_loop()` monitors all scheduler tasks every 30s, auto-restarts crashed tasks up to 3 times, sends ntfy alert when a task exhausts restart budget. Runs separately from supervised tasks.
